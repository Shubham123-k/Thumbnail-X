...
const StyleSelector = ({ value, onChange, isOpen, setIsOpen }: { value: ThumbnailStyle; onChange: (style: ThumbnailStyle) => void; isOpen: boolean; setIsOpen: (Open: boolean) => void }) => {
    ...
    const styleDescription: Record<ThumbnailStyle, string> = {
        'Bold & Graphic': 'High contrast, vibrant colors, strong typography.',
        'Minimalist': 'Clean design with ample white space.',
        'Photorealistic': 'Lifelike images with attention.',
        'Illustrated': 'Hand-drawn illustrated elements and artistic look.',
        'Tech/Futuristic': 'Sleek, modern design with neon accents.',
    }
    ...
    return (
        <div className='relative space-y-3 dark'>
            ...
            <button type='button' onClick={() => setIsOpen(!isOpen)} className='flex w-full item-center justify-between rounded-md border-e px-4 py-3 text-left transition bg-white/8 border-white/10 text-zinc-200 hover:bg-white/12'>
                ...
                <ChevronDownIcon className={['h-9 w-5 text-zinc-400 transition-transform', isOpen && 'rotate-180'].join(' ')} />
            </button>
